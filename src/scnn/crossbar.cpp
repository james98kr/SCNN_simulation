...
CrossBar::CrossBar(ConfigArch* _cfg_arch) {
    cfg_arch = _cfg_arch;
...
// Note that the length of IO_vec stored in port_in need not be F*I (can be less)
...
int CrossBar::hash_function_idx_in_bank(tensor_4D_idx idx) {
    int h = get<2>(idx);
    int w = get<3>(idx);
    int num_of_digits = floor(log10(w)) + 1;
    return (h * num_of_digits) + w;
}
